...
def printHexdump(hexin):
    hexin = hexin.replace(" ","")
    word = 0
    for i in range(0,len(hexin)):
        if i % 32 == 0:
            if i == 0:
                print("0",end='')
            else:
            ...
            if word < 16:
                print("0",end='')
            print(hex(word).lstrip("0x").rstrip("L"),end='')
            print("0",end='')
            word = word+1
            print("  ",end='')
        if i % 2 == 0:
...
def writeHexdump(hexo):
    hexo = hexo.replace(" ","")
    with open(file_path, "w") as file:
        word = 0
        for i in range(0,len(hexo)):
            if i % 32 == 0:
                if i == 0:
                    file.write("0")
                else:
                ...
                if word < 16:
                    file.write("0")
                file.write(hex(word).lstrip("0x").rstrip("L"))
                file.write("0")
                word = word+1
                file.write("  ")
            if i % 2 == 0:
...
if args.outputtype:
    if args.outputtype == "hexdump":
        ...
        result = subprocess.run(runargs, stdout=subprocess.PIPE, input=bytes.getbuffer())
